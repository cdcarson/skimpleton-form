...
import { z } from 'zod';
import { formPath } from './utils.js';
import type { ZParentObject } from './types.js';
...
describe('formPath function', () => {
	...
	describe('round-trip conversions', () => {
		it('should maintain consistency when converting from dot notation', () => {
			...
			const result2 = formPath(testSchema, result1.path as ['profile', 'firstName']);
		...
		it('should maintain consistency when converting from array', () => {
			...
			const result2 = formPath(testSchema, result1.formName as `tags.${number}`);
	...
	describe('3-level deep schemas', () => {
		...
		describe('validation for 3-level paths', () => {
			it('should validate correct 3-level paths', () => {
				...
				expect(() => formPath(deepSchema, 'company.info.employees')).not.toThrow();
			});
